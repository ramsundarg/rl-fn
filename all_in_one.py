...
class Buffer:
    ...
    def update(
        ...
        with tf.GradientTape() as tape:
            ...
            if ep%100 == True:
                mlflow.log_metric("Q loss",critic_loss.numpy())

        critic_grad = tape.gradient(critic_loss, qN.get_trainable_variables())
        ...
        actor_grad = tape.gradient(actor_loss, aN.get_trainable_variables())
...
#@tf.function
def update_target_weights(N,tau):
...
def policy(state, noise_object):
    sampled_actions = tf.squeeze(aN.mu(state,'actual'))
    noise = noise_object()
...
buffer = Buffer(50000, cfg['general_settings']['batch_size'])
...
avg_reward_list = []
mlflow.start_run(run_name=cfg.get('name', "Default Run"))
...
for ep in range(total_episodes):
    ...
    while True:
        ...
        buffer.learn()
        update_target_weights(aN, tau)
        update_target_weights(qN,  tau)
    ...
    if ep%100==0:
        mlflow.log_metric("A_Value",aN.mu(np.expand_dims([0,100], axis=0),'target').numpy(),step)
